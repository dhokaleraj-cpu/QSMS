...
def _render_followups(repo: Repository, complaint: Mapping[str, Any], employees: list[dict], employee_labels: Mapping[str, str], perms: Mapping[str, bool], *, show_heading: bool = True) -> None:
    if show_heading:
...
def render_analysis() -> None:
    ...
    with stage_section("B", 'PROBLEM DEFINITION & IMPACT', 'Define the issue with enough detail to distinguish occurrence, detection and customer/supplier impact.', key="complaints_render_analysis_b"):
    ...
    with stage_section("C", 'IMMEDIATE CORRECTION & CONTAINMENT', key="complaints_render_analysis_c"):
    ...
    with stage_section("D", 'ROOT CAUSE ANALYSIS', 'Separate why the defect occurred from why the control system allowed it to escape.', key="complaints_render_analysis_d"):
    ...
    with stage_section("E", 'EFFECTIVENESS VERIFICATION', key="complaints_render_analysis_e"):
    ...
    with stage_section("F", 'CORRECTIVE / PREVENTIVE ACTION PLAN & RESPONSIBILITY', 'Each action has an accountable owner, target date, completion status, evidence and effectiveness result.', key="complaints_render_analysis_f"):
        actions = repo.select("quality_complaint_actions", eq={"complaint_id": complaint_id}, order_by="action_no", limit=5000)
    ...
    with stage_section("G", 'FINAL CLOSURE & APPROVAL', key="complaints_render_analysis_g"):
