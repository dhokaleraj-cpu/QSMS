...
def _render_standalone_entry(service: InspectionService, perms: dict, parts: dict[str, dict], parties: dict[str, dict], processes: dict[str, dict], stages: dict[str, dict], employee_map: dict[str, str], existing: dict | None) -> None:
    ...
    with stage_section("A", "STANDALONE REPORT CONTEXT", "Master-driven Part / Customer / Material / OSP context. No RMTC or inward transaction linkage is required.", key="dimensional_standalone_context"):
        ...
        saved_plan_id = str((existing or {}).get("inspection_plan_id") or "")
        ...
        if not plan:
            ...
            st.warning("This saved report uses a historical Dimensional layout. QCMS loaded the original layout so the report can be edited without silently changing its specification basis.")
        if not plan:
            if scope == "OSP_STAGE":
...
def render_entry() -> None:
    ...
    # DIRECT DIMENSIONAL EDIT SELECTOR v4.14.6 — always visible
    existing_id = str(st.session_state.get("edit_dimensional_id") or "")
    report_rows = service.dimensional_reports()
    section_bar("NEW / EDIT EXISTING DIMENSIONAL REPORT")
    report_labels = {"": "— Create New Dimensional Report —"}
    report_labels.update({str(row.get("id")): f"{row.get('report_number') or '-'} · {row.get('layout_name_snapshot') or row.get('test_type') or row.get('report_type') or 'Dimensional'} · {row.get('status') or '-'}" for row in report_rows if row.get("id")})
    report_ids = list(report_labels)
    selected_edit_id = st.selectbox(
        "Select Existing Dimensional Report to Edit", report_ids,
        index=report_ids.index(existing_id) if existing_id in report_ids else 0,
        format_func=lambda value: report_labels[value], key="dimensional_direct_edit_selector",
    )
    e1, e2 = st.columns(2, gap="small")
    if e1.button("Load Selected Dimensional Report for Edit", type="primary", width="stretch", disabled=not selected_edit_id or not perms["can_edit"], key="dimensional_direct_edit_load"):
        st.session_state["edit_dimensional_id"] = selected_edit_id
        st.rerun()
    if e2.button("Start New Dimensional Report", width="stretch", key="dimensional_direct_edit_new"):
        st.session_state.pop("edit_dimensional_id", None)
        st.rerun()
    if not report_rows:
        st.info("No saved Dimensional reports are currently visible to this login. The edit control is active and will list reports as soon as records are available to your tenant/permissions.")
    if not perms["can_edit"]:
        st.caption("Your user does not currently have Dimensional Edit permission. Administrator role is not required; module Edit permission is required.")
    existing_id = str(st.session_state.get("edit_dimensional_id") or "")
    ...
    if existing_record and str(existing_record.get("status") or "DRAFT").upper() != "DRAFT":
        section_bar("EDIT SELECTED DIMENSIONAL REPORT")
        password_reopen_for_edit(
            repo=service.repo, table="inspection_reports", record=existing_record, entity_type="DIMENSIONAL_REPORT",
            can_edit=perms["can_edit"], key=f"dimensional_report_edit_unlock_{existing_id}",
            title="Edit Selected Dimensional Report with Password",
        )
    elif existing_record:
        st.success(f"Editing Dimensional Report {existing_record.get('report_number') or existing_id}. Draft fields are editable with your assigned Dimensional Edit permission.")
    ...
    with stage_section("A", 'DIMENSIONAL PENDING LIST', key="dimensional_report_render_entry_a"):
        ...
        if saved_plan_id and all(str(row.get("id")) != saved_plan_id for row in all_plans):
            ...
            if historic_plan and str(historic_plan.get("part_id") or "") == part_id:
                ...
                st.warning("This report uses a historical / no-longer-approved Dimensional layout. QCMS loaded that saved layout for controlled editing instead of changing the report silently.")
        if not all_plans:
            st.warning("No Approved Dimensional Layout is configured for this Part Number, and the saved report layout could not be recovered.")
        ...
        ranked = service.ranked_plans("DIMENSIONAL", part_id)
        ...
        if existing:
            ...
            if saved_plan_id and saved_plan_id not in plan_map:
                st.warning("The originally saved Dimensional layout record could not be found. The current approved layout is shown as a controlled fallback; review before saving.")
        elif selection_mode == "Automatic":
            plan_id = str(recommended["id"])
            st.info(f"Automatically selected: {plan_map.get(plan_id, plan_id)}")
        else:
        ...
        sample_size = c5.number_input("Sample Size", min_value=1, max_value=10, value=sample_size, step=1, disabled=bool(existing) and str((existing or {}).get("status") or "DRAFT").upper() != "DRAFT")
...
def render_records() -> None:
    ...
    if filtered:
        ...
        c1, c2, c3, c4 = st.columns(4, gap="small")
        with c1: st.page_link(st.session_state["_qsms_pages"]["dimensional-entry"], label="Open / Edit Selected Dimensional Report", icon=":material/edit:", width="stretch")
        try:
