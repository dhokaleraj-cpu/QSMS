...
def test_v41440_release_identity_and_schema_baseline():
    assert text("VERSION").strip() in {"4.14.40", "4.14.41", "4.14.42", "4.14.43", "4.14.44", "4.14.45", "4.14.46", "4.14.47", "4.14.48"}
    manifest = json.loads(text("DEPLOYMENT_MANIFEST.json"))
    assert manifest["version"] in {"4.14.40", "4.14.41", "4.14.42", "4.14.43", "4.14.44", "4.14.45", "4.14.46", "4.14.47", "4.14.48"}
    ...
    assert manifest["previous_controlled_release"] in {"4.14.39", "4.14.40", "4.14.41", "4.14.42", "4.14.43", "4.14.44", "4.14.45", "4.14.46", "4.14.47"}
...
def test_android_queues_pending_navigation_until_streamlit_dom_ready():
    ...
    if text("VERSION").strip() in {"4.14.46", "4.14.47", "4.14.48"}:
        for token in (
            'QCMSMobile/0.2.3', 'drawerSection', 'drawerChildButton',
            'appendQueryParameter("native_nav","native")', 'closeDrawer(); navigate(path)',
            'webView.loadUrl(nativeUrl(route))', 'CookieManager.getInstance().flush()',
        ):
...
def test_android_v018_identity_and_footer_remains_removed():
    ...
    java = text("mobile/android_qcms/app/src/main/java/com/fourstar/qcms/MainActivity.java")
    assert any(v in gradle for v in ("versionCode 9", "versionCode 10", "versionCode 11", "versionCode 12", "versionCode 13", "versionCode 14"))
    assert any(v in gradle for v in ("versionName '0.1.8'", "versionName '0.1.9'", "versionName '0.2.0'", "versionName '0.2.1'", "versionName '0.2.2'", "versionName '0.2.3'"))
