...
def test_v41440_release_identity_and_schema_baseline():
    assert text("VERSION").strip() in {"4.14.40", "4.14.41", "4.14.42", "4.14.43", "4.14.44", "4.14.45"}
    manifest = json.loads(text("DEPLOYMENT_MANIFEST.json"))
    assert manifest["version"] in {"4.14.40", "4.14.41", "4.14.42", "4.14.43", "4.14.44", "4.14.45"}
    assert manifest["build"] in {"41440-ANDROID-NAV-READY-QUEUE-BUTTON-BRIDGE", "41441-ANDROID-LAMBDA-COMPILE-PERMANENT-FIX", "41442-LOCAL-JAVAC-OPTIONAL-CI-COMPILE-GUARD", "41443-ANDROID-STREAMLIT-SIDEBAR-NAV", "41444-ANDROID-NATIVE-MENU-SUBMENU-RESTORE", "41445-SHARED-RAW-SOURCE-LAYOUT-CONTROL"}
    assert manifest["previous_controlled_release"] in {"4.14.39", "4.14.40", "4.14.41", "4.14.42", "4.14.43", "4.14.44"}
...
def test_android_queues_pending_navigation_until_streamlit_dom_ready():
    ...
    for token in (
        ...
        'QCMS_NAV_PENDING',
        'MutationObserver',
        '__qcmsNativeNavTimer',
        'setInterval(function(){drain();},250)',
        'installMobileChromeSuppressor();',
        'tryNavigate(route, token, 0)',
...
def test_android_v018_identity_and_footer_remains_removed():
    ...
    java = text("mobile/android_qcms/app/src/main/java/com/fourstar/qcms/MainActivity.java")
    assert any(v in gradle for v in ("versionCode 9", "versionCode 10", "versionCode 11", "versionCode 12"))
    assert any(v in gradle for v in ("versionName '0.1.8'", "versionName '0.1.9'", "versionName '0.2.0'", "versionName '0.2.1'"))
