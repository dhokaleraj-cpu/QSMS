...
def test_v4146_version_build_and_global_strip():
    assert (ROOT / "VERSION").read_text().strip() in {"4.14.6", "4.14.7", "4.14.8", "4.14.9", "4.14.10"}
    app = text("streamlit_app.py")
    assert any(token in app for token in ("4146-LIVE-RUNTIME-DIAGNOSTICS-FORCE-REDEPLOY", "4147-NEXT-STAGE-EMAIL-TEMPLATES-AUTO-OVERDUE-DEPLOY-TARGET", "4148-AUTO-SAFETY-SNAPSHOT-DIRTY-WORKTREE-DEPLOY", "4149-DEPENDENCY-BOOTSTRAP-REMOTE-DEPLOY", "41410-PO-SHIPTO-MASTER-LOGIN-REQUISITIONER"))
...
def test_manifest_declares_forced_redeploy_and_diagnostics():
    ...
    assert any(token in manifest for token in ('"version": "4.14.6"', '"version": "4.14.7"', '"version": "4.14.8"', '"version": "4.14.9"', '"version": "4.14.10"'))
