...
def test_v41438_release_identity_and_schema_baseline():
    assert text("VERSION").strip() in {"4.14.38", "4.14.39", "4.14.40"}
    manifest = json.loads(text("DEPLOYMENT_MANIFEST.json"))
    assert manifest["version"] in {"4.14.38", "4.14.39", "4.14.40"}
    expected_builds = {
...
def test_android_drawer_navigation_preserves_webview_session():
    ...
    assert any(v in java for v in ("QCMSMobile/0.1.6", "QCMSMobile/0.1.7", "QCMSMobile/0.1.8"))
...
def test_android_v016_build_helpers_are_consistent():
    ...
    assert any(v in gradle for v in ("versionCode 7", "versionCode 8", "versionCode 9"))
    assert any(v in gradle for v in ("versionName '0.1.6'", "versionName '0.1.7'", "versionName '0.1.8'"))
    assert "QCMS_Mobile_v${MOBILE_VERSION}_TEST.apk" in helper or "QCMS_Mobile_v0.1.6_TEST.apk" in helper
    assert any(v in readme for v in ("0.1.6", "0.1.7", "0.1.8"))
