...
def test_v41428_release_identity_and_manifest():
    ...
    assert any(marker in app for marker in ('41428-OSP-BATCH-GENEALOGY-TWO-DAY-EXCEL', '41429-RMTC-BEND-CHEM-CASEDEPTH-PERMISSIONS', '41430-RMTC-SUPPLIER-DEDUP-BEND-GLOBAL-SEARCH'))
    manifest = json.loads(text('DEPLOYMENT_MANIFEST.json'))
    assert tuple(map(int, manifest['version'].split('.'))) >= (4, 14, 28)
    assert manifest['build'] in {'41428-OSP-BATCH-GENEALOGY-TWO-DAY-EXCEL', '41429-RMTC-BEND-CHEM-CASEDEPTH-PERMISSIONS', '41430-RMTC-SUPPLIER-DEDUP-BEND-GLOBAL-SEARCH'}
