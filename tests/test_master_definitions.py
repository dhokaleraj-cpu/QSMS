...
def test_fourteen_controlled_master_views() -> None:
    assert len(DEFINITIONS) == 14
    assert len(MASTER_BY_KEY) == 14
    assert set(MASTER_GROUPS) == {"Parties", "Product & Material", "Process & Quality"}


def test_core_master_keys_are_present() -> None:
    required = {
        "customers", "suppliers", "steel_mills", "osp_vendors", "parts",
