...
def test_v4148_release_markers():
    assert (ROOT / "VERSION").read_text().strip() in {"4.14.8", "4.14.9"}
    app = (ROOT / "streamlit_app.py").read_text()
    assert any(token in app for token in ("4148-AUTO-SAFETY-SNAPSHOT-DIRTY-WORKTREE-DEPLOY", "4149-DEPENDENCY-BOOTSTRAP-REMOTE-DEPLOY"))
