...
def test_v41441_release_identity_and_no_schema_change():
    assert text("VERSION").strip() in {"4.14.41", "4.14.42"}
    manifest = json.loads(text("DEPLOYMENT_MANIFEST.json"))
    assert manifest["version"] in {"4.14.41", "4.14.42"}
    assert manifest["build"] in {"41441-ANDROID-LAMBDA-COMPILE-PERMANENT-FIX", "41442-LOCAL-JAVAC-OPTIONAL-CI-COMPILE-GUARD"}
    assert manifest["previous_controlled_release"] in {"4.14.40", "4.14.41", "4.14.42"}
...
def test_android_route_captured_as_final_after_normalization():
    ...
    assert 'QCMSMobile/0.1.9' in java
