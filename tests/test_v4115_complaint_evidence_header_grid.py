...
def test_release_build_and_header_grid():
    assert (ROOT / "VERSION").read_text().strip() in {"4.11.5", "4.11.6", "4.11.7"}
    ui = (ROOT / "core/ui.py").read_text()
    auth = (ROOT / "core/auth.py").read_text()
    assert any(marker in ui for marker in ("4115-COMPLAINT-EVIDENCE-HEADER-GRID", "4116-COMPLAINT-SECTION-COLORS", "4117-COMPLAINT-STAGE-EXPANDERS"))
    assert any(marker in auth for marker in ("4115-COMPLAINT-EVIDENCE-HEADER-GRID", "4116-COMPLAINT-SECTION-COLORS", "4117-COMPLAINT-STAGE-EXPANDERS"))
