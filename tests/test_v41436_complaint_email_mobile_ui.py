...
def test_v41436_release_identity_and_routes():
    assert text("VERSION").strip() in {"4.14.36", "4.14.37", "4.14.38", "4.14.39"}
    manifest = json.loads(text("DEPLOYMENT_MANIFEST.json"))
    assert manifest["version"] in {"4.14.36", "4.14.37", "4.14.38", "4.14.39"}
    assert manifest["build"] in {"41436-COMPLAINT-EMAIL-REGISTERS-REMINDERS-MOBILE-DRAWER", "41437-MOBILE-FULL-NAV-COMPLAINT-CARDS-PO-APPROVAL-DRAFT-EMAIL", "41438-ANDROID-NAV-SESSION-BRIDGE-FOOTER-REMOVE", "41439-ANDROID-CI-SIGNATURE-PERMANENT-FIX"}
...
def test_v41436_android_reference_video_style_navigation():
    ...
    assert any(v in gradle for v in ("versionName '0.1.4'", "versionName '0.1.5'", "versionName '0.1.6'", "versionName '0.1.7'"))
