...
def test_android_stable_mode_is_fullscreen_webview():
    ...
    assert any(v in java for v in ("QCMSMobile/0.2.0", "QCMSMobile/0.2.1"))
...
def test_android_release_identity_is_v020():
    ...
    assert any(v in gradle for v in ("versionCode 11", "versionCode 12"))
    assert any(v in gradle for v in ("versionName '0.2.0'", "versionName '0.2.1'"))
