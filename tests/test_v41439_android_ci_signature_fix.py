...
def test_v41439_release_identity_and_no_schema_change():
    assert text("VERSION").strip() in {"4.14.39", "4.14.40", "4.14.41", "4.14.42", "4.14.43"}
    manifest = json.loads(text("DEPLOYMENT_MANIFEST.json"))
    assert manifest["version"] in {"4.14.39", "4.14.40", "4.14.41", "4.14.42", "4.14.43"}
    assert manifest["build"] in {"41439-ANDROID-CI-SIGNATURE-PERMANENT-FIX", "41440-ANDROID-NAV-READY-QUEUE-BUTTON-BRIDGE", "41441-ANDROID-LAMBDA-COMPILE-PERMANENT-FIX", "41442-LOCAL-JAVAC-OPTIONAL-CI-COMPILE-GUARD", "41443-ANDROID-STREAMLIT-SIDEBAR-NAV"}
    assert manifest["previous_controlled_release"] in {"4.14.38", "4.14.39", "4.14.40", "4.14.41", "4.14.42"}
    assert manifest["database_schema_required"] == "4.14.36"
    assert manifest["schema_change_for_v41439"] is False
    if manifest["version"] in {"4.14.40", "4.14.41", "4.14.42", "4.14.43"}:
...
def test_android_v017_plus_preserves_session_safe_navigation_and_removed_footer():
    ...
    assert any(v in gradle for v in ("versionCode 8", "versionCode 9", "versionCode 10", "versionCode 11"))
    assert any(v in gradle for v in ("versionName '0.1.7'", "versionName '0.1.8'", "versionName '0.1.9'", "versionName '0.2.0'"))
    assert any(v in java for v in ("QCMSMobile/0.1.7", "QCMSMobile/0.1.8", "QCMSMobile/0.1.9", "QCMSMobile/0.2.0"))
