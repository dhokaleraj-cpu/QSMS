...
def test_release_markers_are_v41427():
    assert text("VERSION").strip() in {"4.14.27", "4.14.28", "4.14.29", "4.14.30"}
    app = text("streamlit_app.py")
    assert any(x in app for x in ("41427-FINAL-METLAB-LAYOUT-PO-EMAIL-FIELDS", "41428-OSP-BATCH-GENEALOGY-TWO-DAY-EXCEL", "41429-RMTC-BEND-CHEM-CASEDEPTH-PERMISSIONS", "41430-RMTC-SUPPLIER-DEDUP-BEND-GLOBAL-SEARCH"))
    assert any(x in text("DEPLOYMENT_MANIFEST.json") for x in ('"version": "4.14.27"', '"version": "4.14.28"', '"version": "4.14.29"', '"version": "4.14.30"'))
