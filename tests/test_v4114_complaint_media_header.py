...
def test_release_and_build():
    assert (ROOT / "VERSION").read_text().strip() in {"4.11.4", "4.11.5", "4.11.6"}
    assert any(marker in (ROOT / "core/ui.py").read_text() for marker in ("4114-COMPLAINT-MEDIA-HEADER-FIX", "4115-COMPLAINT-EVIDENCE-HEADER-GRID", "4116-COMPLAINT-SECTION-COLORS"))
    assert any(marker in (ROOT / "core/auth.py").read_text() for marker in ("4114-COMPLAINT-MEDIA-HEADER-FIX", "4115-COMPLAINT-EVIDENCE-HEADER-GRID", "4116-COMPLAINT-SECTION-COLORS"))
